...
// Message type definition to maintain compatibility
interface Message {
...
export async function selectContext(props: {
  ...
  // Use Mastra service to select relevant files
  const resp = await mastraService.generateText({
...
---
  ...
  const includeFiles =
    updateContextBuffer[1]
      ...
      ?.map((x: string) => x.replace('<includeFile path="', '').replace('"', '')) || [];
  const excludeFiles =
    updateContextBuffer[1]
      .match(/<excludeFile path="(.*?)"/gm)
      ?.map((x: string) => x.replace('<excludeFile path="', '').replace('"', '')) || [];

  const filteredFiles: FileMap = {};
  excludeFiles.forEach((path: string) => {
    delete contextFiles[path];
  });
  includeFiles.forEach((path: string) => {
    let fullPath = path;
  ...
  if (totalFiles == 0) {
    throw new Error(`Seyi failed to select files`);
  }
